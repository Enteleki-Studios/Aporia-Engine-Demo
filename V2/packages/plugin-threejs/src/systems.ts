...
export const syncThreeSystem = createSystem<{
    renderer: Renderer
    objectManager: ResourceManager<Group, Object3D>
}>('threejs sync', ({ renderer, objectManager }) => (world: World) => {
...
const LABEL = 'render'
export const renderSystem = createSystem<{ renderer: Renderer }>(
    LABEL,
    ({ renderer }) =>
        (world: World) => {
            renderer.render()
            ...
            world.stats.systemsStats[LABEL].extra.calls =
                renderer.renderer.info.render.calls
            world.stats.systemsStats[LABEL].extra.triangles =
                renderer.renderer.info.render.triangles
            world.stats.systemsStats[LABEL].extra.geometries =
                renderer.renderer.info.memory.geometries
            world.stats.systemsStats[LABEL].extra.textures =
                renderer.renderer.info.memory.textures
